...
DOWNLOAD_URL="https://github.com/hashicorp/terraform/releases/download/v${TF_VERSION}/${ZIP_NAME}"

mkdir -p "${TOOLS_DIR}"

echo "Downloading Terraform ${TF_VERSION} from ${DOWNLOAD_URL}" >&2
curl -fsSL -H "Accept: application/octet-stream" "${DOWNLOAD_URL}" -o "${TOOLS_DIR}/${ZIP_NAME}"
